...
# def trigers():
#     try:
#         query = list(Trig.query.all())
#         return render_template("trigers.html", query=query)
#     except Exception as e:
#         print(e)
#         # Handle the exception or print an error message if needed
#         flash("Error fetching data", "danger")
#         return render_template("trigers.html", query=[])
...
def slotbooking():
    ...
    if request.method == "POST":
        ...
        if dbb:
            seat = 0  # Initialize seat count to 0, adjust based on your logic
            bedtype = bedtype

            if bedtype == "NormalBed":
                seat = dbb.normalbed
                # Update seat count and perform other logic
            ...
            if seat > 0:
                res = Bookingpatient(srfid=srfid, bedtype=bedtype, hcode=hcode, spo2=spo2,
...
def discharge():
    if request.method == 'POST':
        ...
        if patient:
            hospital_data = Hospitaldata.query.filter_by(hcode=patient.hcode).first()
            if hospital_data:
                if patient.bedtype == 'NormalBed':
                    hospital_data.normalbed += 1
                elif patient.bedtype == 'HICUBed':
                    hospital_data.hicubed += 1
                elif patient.bedtype == 'ICUBed':
                    hospital_data.icubed += 1
                elif patient.bedtype == 'VENTILATORBed':
                    hospital_data.vbed += 1
                db.session.delete(patient)
                db.session.commit()
                flash("Patient discharged successfully and bed count updated.", "success")
                return redirect('/pdetails')
            else:
                flash("Hospital data not found for the patient.", "danger")
        else:
